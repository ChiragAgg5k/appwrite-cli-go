...
type UsageDataPoint struct {
	...
	// Node ordinal when broken down by `ordinal`. A stable per-node identity, not
	// a role: ordinal 0 is the first member created, and a failover can leave the
	// primary on any ordinal. Read the role from the database's replicas
	// endpoint.
